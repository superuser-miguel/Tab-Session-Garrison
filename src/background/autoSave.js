...
async function isChangedAutoSaveSession(session) {
  ...
  const lastRegularSession = await getLatestSessionByTag("regular");
  if (!lastRegularSession) return true;
  ...
  //前回保存時とタブが異なればtrue
  return tabsToString(lastRegularSession) != tabsToString(session);
}
